...
if (isset($_GET['cancel'])) {
	mysqli_query($conn, "update appointment set tutor_status='0' where id ='" . $_GET['id'] . "'");
	$_SESSION['msg'] = "Appointment not approved !!";
}

if (isset($_GET['approve'])) {
	mysqli_query($conn, "update appointment set tutor_status='2' where id ='" . $_GET['id'] . "'");
...
<head>
	<title>Tutor | Appointment History</title>
						...
						<div class="row">
							<div class="col-md-12">
								...
								<table class="table table-hover" id="sample-table-1">
									...
									<tbody>
										<?php
										$sql = mysqli_query($conn, "select stud.name as name,appointment.*  from appointment join stud on stud.id=appointment.userId where appointment.tutor='" . $_SESSION['id'] . "'");
										$cnt = 1;
											...
											<tr>
												...
												<td>
													<div class="visible-md visible-lg hidden-sm hidden-xs">
														<?php if (($row['userStatus'] == 1) && ($row['centre_status'] == 1)) { ?>


															<a href="consultation.php?id=<?php echo $row['id'] ?>&cancel=update" onClick="return confirm('Are you sure you want to cancel this appointment ?')" class="btn btn-transparent btn-xs tooltips" title="Cancel Appointment" tooltip-placement="top" tooltip="Remove">Cancel</a>
															<a href="consultation.php?id=<?php echo $row['id'] ?>&approve=update" onClick="return confirm('Are you sure you want to approve this appointment ?')" class="btn btn-transparent btn-xs tooltips" title="Approve Appointment" tooltip-placement="top" tooltip="Remove">Approve</a>
														<?php } else if (($row['userStatus'] == 1) && ($row['centre_status'] == 2)) { ?>
															<a href="consultation.php?id=<?php echo $row['id'] ?>&cancel=update" onClick="return confirm('Are you sure you want to cancel this appointment ?')" class="btn btn-transparent btn-xs tooltips" title="Cancel Appointment" tooltip-placement="top" tooltip="Remove">Cancel</a>
														<?php } else if (($row['userStatus'] == 1) && ($row['centre_status'] == 0)) { ?>
															<a href="consultation.php?id=<?php echo $row['id'] ?>&approve=update" onClick="return confirm('Are you sure you want to approve this appointment ?')" class="btn btn-transparent btn-xs tooltips" title="Approve Appointment" tooltip-placement="top" tooltip="Remove">Approve</a>
														<?php } ?>
